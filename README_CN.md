...
- **浏览器自动化插件** — 原生 Chrome/Edge 扩展，支持直接操作 DOM、提取内容和管理标签页
...
VibeWorker 提供了一个原生的浏览器扩展，赋予 Agent 直接控制浏览器完成复杂网络任务的能力：

- **DOM 交互**：代你点击元素、输入文本、提交表单
- **内容提取**：直接从当前标签页提取干净的 Markdown 和结构化数据
- **标签页管理**：自动打开、关闭和导航标签页
- **无缝升级**：集成版本检测并在有新版本扩展时进行 UI 提示报警

**安装方法**：
1. 打开 `chrome://extensions/` 或是 `edge://extensions/`
2. 开启右上角的 **开发者模式**
3. 点击 **加载已解压的扩展程序**，并选择你本地 VibeWorker 项目下的 `/extension` 目录
